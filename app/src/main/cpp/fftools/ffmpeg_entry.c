...
FILE *vstats_file;
static _Atomic int g_abort_flag = 0;
...
void ffmpeg_request_abort(void) {
    g_abort_flag = 1;
}
...
static void ffmpeg_cleanup(int ret) {
    if (do_benchmark) {
        ...
        av_log(NULL, AV_LOG_INFO, "bench: maxrss=%"
        PRId64
        "KiB\n", maxrss);
    }
...
static void print_report(int is_last_report, int64_t timer_start, int64_t cur_time, int64_t pts) {
    ...
    for (OutputStream *ost = ost_iter(NULL); ost; ost = ost_iter(ost)) {
        ...
        if (!vid && ost->type == AVMEDIA_TYPE_VIDEO) {
            ...
            av_bprintf(&buf, "frame=%5"
            PRId64
            " fps=%3.*f q=%3.1f ",
                    frame_number, fps < 9.95, fps, q);
            av_bprintf(&buf_script, "frame=%"
            PRId64
            "\n", frame_number);
    ...
    if (pts == AV_NOPTS_VALUE) {
        ...
        PRId64
        "\n", pts);
        av_bprintf(&buf_script, "out_time=%s%02"
        PRId64
        ":%02d:%02d.%06d\n",
    ...
    PRId64
    " drop=%"
    PRId64, nb_frames_dup, nb_frames_drop);
    av_bprintf(&buf_script, "dup_frames=%"
    PRId64
    "\n", nb_frames_dup);
    av_bprintf(&buf_script, "drop_frames=%"
    PRId64
    "\n", nb_frames_drop);
